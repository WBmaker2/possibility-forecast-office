...
export function MissionFlow(props: Props) {
  ...
  if (stage === "first-forecast") return <section className="step panel"><h1 ref={headingRef} tabIndex={-1}>첫 자료로 예보해요</h1><SourceBadge kind={activity.sourceKind} />{activity.sourceKind === "known-model" ? <KnownModelComparison observed={first} known={displayFraction} batchCount={1} /> : <p>첫 자료에서 실제로 나온 결과의 비율: <FractionView fraction={displayFraction} /></p>}<NumberLine fraction={displayFraction} kind="first" label="첫 예보에 쓸 분수" /><ForecastChoices kind={activity.sourceKind} value={props.answers.firstForecast} onChange={(firstForecast) => props.onAnswers({ firstForecast })} /><button className="primary" data-testid="first-forecast-confirm" onClick={() => props.onForecast(1)}>첫 예보 정하기</button></section>;
  if (stage === "provisional-decision") return <section className="step panel"><h1 ref={headingRef} tabIndex={-1}>첫 자료를 보고 첫 선택해요</h1><DecisionChoices activity={activity} batchCount={1} value={props.answers.firstDecision} onChange={(firstDecision) => props.onAnswers({ firstDecision })} /><button className="primary" data-testid="first-decision-confirm" onClick={props.onFirstDecision}>첫 선택 확인</button></section>;
  if (stage === "reveal") return <section className="step panel"><h1 ref={headingRef} tabIndex={-1}>새 자료가 도착했어요</h1><p>새 자료만 보지 말고 첫 자료와 모두 합쳐서 살펴봐요.</p><ResultBoards activity={activity} batchIndex={1} heading="새 자료" /><button className="primary" data-testid="reveal-next" onClick={() => props.onMove("cumulative")}>모두 합친 자료 세기</button></section>;
  if (stage === "cumulative") return <section className="step panel"><h1 ref={headingRef} tabIndex={-1}>모두 합친 자료를 세어 봐요</h1><p>새 자료만 쓰지 말고 첫 자료와 새 자료를 모두 합친 횟수를 고르세요.</p><CountPicker activity={activity} counts={props.finalCounts} firstCounts={props.firstCounts} selections={props.cumulativeSelections} onSelections={props.onCumulativeSelections} prefix="cumulative" /><button className="primary" data-testid="cumulative-count-confirm" onClick={props.onCumulative}>모두 합친 횟수 확인</button></section>;
