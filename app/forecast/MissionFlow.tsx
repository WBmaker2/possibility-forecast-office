...
export function MissionFlow(props: Props) {
  ...
  if (stage === "condition") return <section><h1 className="step-heading" ref={headingRef} tabIndex={-1}>{activity.title}</h1><ActivityCondition activity={activity} /><button className="primary" data-testid="condition-confirm" onClick={() => props.onMove("first-count")}>찾을 결과를 확인했어요</button></section>;
  ...
  if (stage === "first-forecast") return <section className="step panel"><h1 ref={headingRef} tabIndex={-1}>첫 자료로 예보해요</h1><SourceBadge kind={activity.sourceKind} /><p>{activity.sourceKind === "known-model" ? "신호판의 칸으로 계산한 가능성" : "첫 자료에서 실제로 나온 결과의 비율"}: <FractionView fraction={displayFraction} /></p>{activity.activityKind === "tutorial" && <div className="comparison-note"><p>신호판의 칸은 파랑 4칸, 모두 8칸이라서 4/8이에요.</p><p>실제로 나온 결과는 파랑이 6번 중 4번이라서 4/6이에요.</p></div>}<NumberLine fraction={displayFraction} kind="first" label="첫 예보에 쓸 분수" /><ForecastChoices kind={activity.sourceKind} value={props.answers.firstForecast} onChange={(firstForecast) => props.onAnswers({ firstForecast })} /><button className="primary" data-testid="first-forecast-confirm" onClick={() => props.onForecast(1)}>첫 예보 정하기</button></section>;
  if (stage === "provisional-decision") return <section className="step panel"><h1 ref={headingRef} tabIndex={-1}>첫 자료를 보고 첫 선택해요</h1><DecisionChoices activity={activity} value={props.answers.firstDecision} onChange={(firstDecision) => props.onAnswers({ firstDecision })} /><button className="primary" data-testid="first-decision-confirm" onClick={props.onFirstDecision}>첫 선택 확인</button></section>;
  if (stage === "reveal") return <section className="step panel"><h1 ref={headingRef} tabIndex={-1}>새 자료가 도착했어요</h1><p>새 자료만 보지 말고 첫 자료와 모두 합쳐서 살펴봐요.</p><div className="stream-list">{activity.streams.map((stream) => <ResultBoard key={stream.id} activity={activity} stream={stream} batch={stream.batches[1]} heading="새 자료" />)}</div><button className="primary" data-testid="reveal-next" onClick={() => props.onMove("cumulative")}>모두 합친 자료 세기</button></section>;
  if (stage === "cumulative") return <section className="step panel"><h1 ref={headingRef} tabIndex={-1}>모두 합친 자료를 세어 봐요</h1><p>새 자료만 쓰지 말고 첫 자료와 새 자료를 모두 합친 횟수를 고르세요.</p><CountPicker activity={activity} counts={props.finalCounts} selections={props.cumulativeSelections} onSelections={props.onCumulativeSelections} prefix="cumulative" /><button className="primary" data-testid="cumulative-count-confirm" onClick={props.onCumulative}>모두 합친 횟수 확인</button></section>;
  if (stage === "revised-forecast") return <section className="step panel"><h1 ref={headingRef} tabIndex={-1}>모두 합친 자료로 최종 예보해요</h1><SourceBadge kind={activity.sourceKind} /><p>{activity.sourceKind === "known-model" ? "신호판의 칸은 그대로예요" : "모두 합친 실제 결과의 비율"}: <FractionView fraction={finalDisplay} /></p><NumberLine fraction={finalDisplay} kind="final" label="최종 예보에 쓸 분수" /><ForecastChoices kind={activity.sourceKind} value={props.answers.finalForecast} onChange={(finalForecast) => props.onAnswers({ finalForecast })} /><button className="primary" data-testid="final-forecast-confirm" onClick={() => props.onForecast(2)}>최종 예보 정하기</button></section>;
...
function CountStep({ activity, headingRef, firstCounts, firstSelections, onFirstSelections, onCount }: Props & { batchCount: 1; selections: CountSelections; onSelections: (value: CountSelections) => void; onConfirm: () => void }) {
  return <section className="step panel"><h1 ref={headingRef} tabIndex={-1}>첫 자료를 세어 봐요</h1><p>결과 그림에서 찾을 결과가 몇 번인지와 모두 몇 번인지 골라요.</p><div className="stream-list">{activity.streams.map((stream) => <ResultBoard key={stream.id} activity={activity} stream={stream} batch={stream.batches[0]} heading="첫 자료" />)}</div><CountPicker activity={activity} counts={firstCounts} selections={firstSelections} onSelections={onFirstSelections} prefix="first" /><button className="primary" data-testid="first-count-confirm" onClick={onCount}>횟수 확인</button></section>;
}
