...
function SourceOverclaimPractice({ state, onWrong, onRight }: { state: { none: boolean; all: boolean; feedback: Feedback }; onWrong: () => void; onRight: (key: "none" | "all") => void }) {
  ...
  return <section id="concept-practice" className="overclaim-practice concept-practice-anchor" aria-labelledby="overclaim-title"><h2 id="overclaim-title">나온 결과 말 고르기</h2><p>지금 나온 횟수만 보고 다음 결과를 딱 정할 수는 없어요.</p><div className="overclaim-options"><fieldset><legend>10번 해 보니 0번 나왔어요.</legend><button type="button" data-testid="source-overclaim-wrong-none" onClick={onWrong}>불가능</button><button type="button" data-testid="source-overclaim-right-none" onClick={() => onRight("none")}>이번 10번에서는 아직 나오지 않았어요</button>{state.none && <strong>확인</strong>}</fieldset><fieldset><legend>10번 해 보니 10번 나왔어요.</legend><button type="button" data-testid="source-overclaim-wrong-all" onClick={onWrong}>반드시</button><button type="button" data-testid="source-overclaim-right-all" onClick={() => onRight("all")}>이번 10번에서는 매번 나왔어요</button>{state.all && <strong>확인</strong>}</fieldset></div>{state.feedback && <div ref={feedbackRef} className={`feedback-banner ${state.feedback.tone}`} data-testid="source-overclaim-feedback" role={state.feedback.tone === "error" ? "alert" : "status"} tabIndex={-1}><strong>{state.feedback.tone === "error" ? "다시 확인해요" : "잘했어요"}</strong><span>{state.feedback.message}</span></div>}</section>;
}
